...
const orderSchema = new mongoose.Schema(
  {
    ...
    // Step 1: Fuel Type - no enum, just validate manually
    fuelType: {
      type: String,
      required: true,
      trim: true,
      validate: {
        validator: function (v) {
          ...
          return validFuelTypes.includes(v);
        },
        message: (props) => `${props.value} is not a valid fuel type. Allowed: Petrol, Diesel, Petrol (95 Octane)`,
      },
    },
...
orderSchema.statics.calculatePrice = function (fuelType, quantity) {
  // Normalize fuel type to match price keys
  const normalizedFuel = fuelType?.trim() || "";
  ...
  const pricePerLiter = prices[normalizedFuel] || 0;
